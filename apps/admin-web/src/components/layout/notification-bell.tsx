...
export function NotificationBell({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className="relative inline-flex size-9 shrink-0 items-center justify-center rounded-lg border border-white/12 text-white/70 transition-colors hover:bg-white/[0.07] hover:text-white focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
        aria-label={
          items.length === 0
            ...
            {items.map((item) => (
              ...
              >
                ...
                {item.href && (
                  <Link
                    href={item.href}
                    className="mt-1.5 inline-flex text-xs font-semibold text-admin-deep hover:underline"
                  >
