import { advanceRainState, isAutomaticRainActive, LAJINHA, RAIN_MAX_AGE_MS } from './rain-policy';
...
describe('regra de chuva de Lajinha', () => {
  it('liga pela chuva atual, incluindo garoa e pancadas', () => {
    for (const code of [51, 53, 55, 61, 63, 65, 80, 81, 82, 95]) {
    ...
    const state = advanceRainState(
      { ...weather(), current: { time: start / 1000, rain: 0, showers: 0.1, weather_code: 3 } },
      null,
      start,
    );
