# Changelog de engenharia — MOTOboyCity

> Registro APPEND-ONLY do que foi feito, em ordem cronológica. Cada entrada
> guarda a decisão, o motivo, os arquivos e as validações daquele recorte.
>
> **Este arquivo não descreve o estado atual do sistema.** Para isso use
> `agent-handoff.md` (o que está valendo hoje) e `architecture.md` (como o
> sistema é organizado). Uma entrada aqui era verdade na data em que foi
> escrita; consulte-a como história, não como referência.
>
> As entradas até 2026-08-20 vinham de uma seção que era mantida em ordem
> decrescente. O conteúdo não foi alterado — apenas reordenado para ficar
> cronológico como o resto.
...
### 2026-09-15 - CRUD Administrativo de Pedidos Conclu�dos

Implementado CRUD no painel do administrador para cancelar pedidos ou alterar seus valores mesmo ap�s o status COMPLETED, desde que a fatura n�o tenha sido gerada (invoiceId === null) e o repasse n�o tenha sido creditado ou cancelado (status === PENDING). Atualiza��es e cancelamentos ocorrem dentro de uma transa��o at�mica que ajusta o saldo bloqueado do motoboy usando c�lculos seguros (Prisma.Decimal.minus/increment/decrement), prevenindo concorr�ncia com optimistic locking via updateMany do WalletTransaction. As a��es exigem um motivo e criam logs de auditoria detalhados (AdministrativeAudit e DeliveryStatusHistory).
